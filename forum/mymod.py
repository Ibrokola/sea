...
# 	""" An instance on which voting is probable 
# 	"""
...
# 		# This saves the vote first
# 		vote = Vote(content_object=self, voter=user,
# 			type_of_vote=type_of_vote)
# 		vote.save()

# 		score_delta = 0
# 		# Followed by updating denormalized columns
...
# 	# def get_post_started_by_me(self, post_id, user=None):
# 	# 	post_me = Post.objects.select_related("author").get(pk=post_id)
		


# 	def recent_posts_with_my_votes(self, user=None):
# 		posts = Post.objects.annotate(score=F('upvotes') - F('downvotes')).select_related("author").order_by("-submission_time")[:50]
# 		if user:
...
# 		objects = Vote.objects.only('object_id', 'type_of_vote').filter(content_type=post_type.id, object_id__in=post_ids, voter=user)
...
# 	def get_absolute_url(self): #change id to slug later, it's more robust
...
# 				WHERE c.post_id = %s
# 				ORDER BY c.wbs
# 			""", [comment_type.id, user_id, comment_type.id, user_id, post_id]
# 			)
...
# 				comment.is_upvoted = True if row[12] else False
...
# 	submission_time = models.DateTimeField(auto_now_add=True)
# 	wbs 			= models.CharField(max_length=30) #wbs helps track the comment tree
